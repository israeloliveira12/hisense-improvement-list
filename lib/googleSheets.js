...
const MAX_FOTOS_POR_LADO = 6; // mesmo limite do gerador Python original (grade 3x2)
...
export async function getAcoes() {
  ...
  return grupos.map(({ row, acao, itens }) => {
    ...
    return {
      ...
      statusRaw: acao["Status"] || "Open", // valor de verdade (Open/Under Review/Closed), pro editor
      occur: serialToDateBR(acao["Occur. Date"]),
      deadlineOriginal: serialToDateBR(acao["Deadline"]),
      ...
      auditor: acao["Audit"] || "",
      // "Foto Before"/"Foto Improvement" guardam uma lista de IDs do Drive
      // separada por virgula agora (antes era 1 ID so) -- a primeira e a
      // foto "principal" (grande, com zoom/posicao ajustavel), o resto
      // aparece como miniaturas extras. Formato antigo (1 ID sem virgula)
      // continua funcionando igual, vira uma lista de 1.
      fotoBeforeId: parseFotoIds(det["Foto Before"])[0] || null,
      fotoImprovementId: parseFotoIds(det["Foto Improvement"])[0] || null,
      fotosBeforeExtra: parseFotoIds(det["Foto Before"]).slice(1),
      fotosImprovementExtra: parseFotoIds(det["Foto Improvement"]).slice(1),
      fotoBeforeAjuste: parseAjuste(det["Foto Before Ajuste"]),
      fotoImprovementAjuste: parseAjuste(det["Foto Improvement Ajuste"]),
...
export async function getDashboardStats() {
  ...
  const agingList = []; // {no, item, dias} -- so acoes Open/Under Review
  ...
  grupos.forEach(({ acao, itens }) => {
    ...
    if (!porDeptDetalhe.has(dept)) porDeptDetalhe.set(dept, { open: 0, closed: 0, delayed: 0 });
