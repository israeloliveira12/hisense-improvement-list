...
// Adiciona o PRIMEIRO item de investimento de uma acao, escrevendo direto
// no bloco V:AL da propria linha-ancora (que fica vazio quando a acao
// ainda nao tem nenhum item -- sem precisar inserir linha nem mexer em
// merge). Se a acao JA tem item(ns), nao ha linha "sobrando" pra escrever.
//
// ATENCAO -- ja existiu aqui uma versao que inseria uma linha nova +
// reaplicava a mescla do bloco Acao pra permitir 2+ itens (insertDimension
// + unmergeCells/mergeCells via batchUpdate). Foi REVERTIDA depois de
// causar PERDA DE DADOS em producao: uma acao inteira sumiu da planilha
// (60 -> 59) ao adicionar um 2o item numa acao que ja tinha 1 aprovado.
// Nao foi possivel confirmar com certeza a causa exata so lendo o codigo
// (interacao entre inheritFromBefore, o comportamento de "so a celula
// superior-esquerda sobrevive a um merge" e as 5 colunas congeladas da
// aba) sem uma copia de teste pra reproduzir -- entao, em vez de arriscar
// mais uma tentativa as cegas numa planilha de producao, voltou pro
// comportamento seguro: pede pra fazer manualmente na planilha quando ja
// existe item. Antes de tentar de novo, isso precisa ser testado numa
// COPIA da planilha, nao direto em producao.
export async function addInvestimentoItem(no, { item, quantity, unitCost, supplier }) {
  ...
  if (alvo.itens.length > 0) {
    throw new Error("Esta ação já tem item de investimento — adicionar outro exige uma linha nova na planilha; peça pra alguém inserir manualmente por enquanto.");
  }
