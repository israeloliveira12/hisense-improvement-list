...
function investimentoInfo(itens) {
  ...
  const approval = itens[0]["Request Approval"];
  return {
    total: Math.round(total * 100) / 100,
    situacao: approval === "Approved" ? "Approved" : approval === "Declined" ? "Declined" : "Pending",
  };
}
...
function investimentoDisplay(itens) {
  ...
  const total = itens.reduce((acc, it) => acc + (Number(it["Quantity"]) || 0) * (Number(it["Unit cost (BRL)"]) || 0), 0);
  const approval = itens[0]["Request Approval"] || "";
  const situacao = { Approved: "Approved", Declined: "Declined" }[approval] || "Pending";
  ...
  await sheets.spreadsheets.batchUpdate({
    ...
    requestBody: {
      requests: [
        ...
        // reaplica a mescla do bloco Acao (B:U, colunas 1..20 em indice 0)
        // cobrindo a linha nova -- desfaz a mescla antiga primeiro pra
        // nunca colidir com um merge existente de extensao diferente.
        {
          unmergeCells: {
            range: { sheetId, startRowIndex: alvo.row - 1, endRowIndex: ultimaLinhaAtual, startColumnIndex: 1, endColumnIndex: 21 },
          },
        },
        {
          mergeCells: {
            range: { sheetId, startRowIndex: alvo.row - 1, endRowIndex: novaLinha, startColumnIndex: 1, endColumnIndex: 21 },
            mergeType: "MERGE_ALL",
          },
        },
      ],
