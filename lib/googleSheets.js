...
// -- precisaria inserir uma linha nova e deslocar tudo abaixo, arriscado
// demais pra fazer as cegas numa planilha de producao; por enquanto pede
// pra fazer direto na planilha nesse caso (mesmo aviso que ja existia).
// "#" (V) e numerado corrido em TODA a planilha, nao reinicia por acao --
// por isso o maximo e calculado sobre todos os grupos, nao so o alvo.
export async function addInvestimentoItem(no, { item, quantity, unitCost, supplier }) {
  ...
  const data = Object.entries(valores)
    ...
    .map(([col, v]) => ({ range: `${TAB_IMPROVEMENT}!${col}${alvo.row}`, values: [[v]] }));
  ...
  // Ter um item de investimento so faz sentido se a acao precisa de
  // investimento -- garante que a flag (S) reflita isso, mesmo que quem
  // criou a acao nao tenha marcado (a criacao pelo site nem pergunta mais
  // isso, ver createAcao).
