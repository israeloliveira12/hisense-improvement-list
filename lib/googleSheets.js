...
// Itens de investimento moram numa aba propria, no MESMO padrao ja usado
// por PPT_Passos/PPT_Detalhes: 1 linha por item, ligada a acao so pela
// coluna "No." (chave estrangeira de texto), sem merge nenhum. Antes
// ficavam dentro da propria "Improvement List" (colunas V:AL), usando
// celulas MESCLADAS pra representar "1 acao com N itens" -- foi essa
// mesclagem que causou perda de dados em producao (ver git log). Migrado
// em 2026 (dados antigos permanecem intactos, so nao sao mais lidos).
const TAB_INVESTIMENTOS = "PPT_Investimentos";
...
// Colunas da aba PPT_Investimentos (A = No., a chave estrangeira).
...
// Le o bloco B4:U da Improvement List -- desde que os itens de investimento
// migraram pra aba propria (PPT_Investimentos, ver TAB_INVESTIMENTOS), cada
// linha com "No." preenchido e uma acao COMPLETA e independente. Sem
// celula mesclada, sem linha de continuacao, sem agrupamento nenhum --
// cada linha e uma acao, ponto. (As colunas V:AL antigas continuam
// existindo na planilha com o historico pre-migracao, mas nao sao mais
// lidas por aqui.)
async function readImprovementListRaw() {
  const rows = await readRange(`${TAB_IMPROVEMENT}!B4:U1000`);
  const grupos = [];

  rows.forEach((row, i) => {
    const absRow = 4 + i;
    const noCel = row[2]; // D = No. (B=0, C=1, D=2)
    if (noCel === undefined || noCel === null || noCel === "") return;

    const acao = {};
    ACTION_COLS.forEach((nome, idx) => (acao[nome] = row[idx] ?? null));
    grupos.push({ row: absRow, acao });
  });

  return grupos;
}

// Le PPT_Investimentos e agrupa por "No." (mesmo padrao ja usado pros
// passos em getAcoes()/getDashboardStats()). Devolve um Map No. -> itens[].
async function readInvestimentosPorNo() {
  const { rows } = await readSimpleTab(TAB_INVESTIMENTOS);
  const porNo = new Map();
  rows.forEach((r) => {
    const key = normNo(r["No."]);
    if (!porNo.has(key)) porNo.set(key, []);
    porNo.get(key).push(r);
  });
  return porNo;
}
...
// Rotacao por foto -- guardada a parte da lista de ordem, formato esparso
// "id:90,id:270" (so entra quem tem rotacao != 0, entao fotos antigas sem
// nada nessa coluna continuam simplesmente sem rotacao).
function parseRotacoes(str) {
  const out = {};
  if (!str) return out;
  String(str).split(",").forEach((par) => {
    const [id, deg] = par.split(":");
    if (id && deg && !Number.isNaN(Number(deg))) out[id.trim()] = Number(deg);
  });
  return out;
}

// Tipo de anexo (foto/video/documento) + nome original do arquivo --
// mesmo formato esparso da rotacao ("id:tipo:nomeCodificado,..."), so
// entra quem NAO e foto (o tipo default, pra nao precisar de entrada
// nenhuma nos milhares de fotos ja existentes). Nome vem com
// encodeURIComponent pra sobreviver mesmo se tiver virgula/dois-pontos.
function parseAnexoMeta(str) {
  const out = {};
  if (!str) return out;
  String(str).split(",").forEach((par) => {
    // 4o campo (poster) e OPCIONAL e so existe pra video: e o fileId de
    // uma imagem no Drive com um quadro do proprio video, gerada no
    // navegador na hora do upload (ver extrairCapaDeVideo em
    // PresentationClient.js). Entrada antiga, com 3 campos, continua
    // valendo -- fica sem capa e cai no placeholder de antes.
    const [id, tipo, nomeCod, posterId] = par.split(":");
    if (id && tipo) {
      out[id.trim()] = {
        tipo: tipo.trim(),
        nome: nomeCod ? decodeURIComponent(nomeCod) : "",
        poster: posterId ? posterId.trim() : "",
      };
    }
  });
  return out;
...
// Olha a aprovacao de TODOS os itens da acao, nao so o primeiro -- com 1
// item so era irrelevante, mas com 2+ itens (agora possivel, ver
// addInvestimentoItem) o primeiro item podia estar Approved e o segundo
// ainda Pending, e o resumo mentia dizendo "Approved" pra acao inteira.
// "Partial" e um 4o estado pra quando os itens discordam entre si (nem
// todos aprovados, nem todos recusados, mas pelo menos 1 ja decidido).
function situacaoConsolidada(itens) {
  const aprovados = itens.filter((it) => it["Request Approval"] === "Approved").length;
  const recusados = itens.filter((it) => it["Request Approval"] === "Declined").length;
  if (aprovados === itens.length) return "Approved";
  if (recusados === itens.length) return "Declined";
  if (aprovados > 0 || recusados > 0) return "Partial";
  return "Pending";
}

// Versao ESTRUTURADA do resumo de investimento. A string pronta
// (investimentoDisplay) continua existindo porque o .pptx e documento
// oficial e fica em ingles; a TELA usa isto aqui pra montar o texto no
// idioma escolhido.
function investimentoInfo(itens) {
  if (!itens.length) return null;
  const total = itens.reduce((acc, it) => acc + (Number(it["Quantity"]) || 0) * (Number(it["Unit cost (BRL)"]) || 0), 0);
  return {
    total: Math.round(total * 100) / 100,
    situacao: situacaoConsolidada(itens),
  };
}
...
function investimentoDisplay(itens) {
  if (!itens.length) return null;
  const total = itens.reduce((acc, it) => acc + (Number(it["Quantity"]) || 0) * (Number(it["Unit cost (BRL)"]) || 0), 0);
  const situacao = situacaoConsolidada(itens);
  return formatBRL(total) + ` · ${situacao}`;
}
...
export async function getAcoes() {
  const [grupos, detalhes, passos, investimentosPorNo] = await Promise.all([
    readImprovementListRaw(),
    readSimpleTab(TAB_DETALHES),
    readSimpleTab(TAB_PASSOS),
    readInvestimentosPorNo(),
  ]);
  ...
  return grupos.map(({ row, acao }) => {
    const no = normNo(acao["No."]);
    const itens = investimentosPorNo.get(no) || [];
    ...
    return {
      ...
      investmentInfo: investimentoInfo(itens),
      ...
      fotosBeforeRotacao: parseRotacoes(det["Foto Before Rotacao"]),
      fotosImprovementRotacao: parseRotacoes(det["Foto Improvement Rotacao"]),
      // Mesma lista de IDs acima, so que agora pode ter video/documento
      // misturado com foto -- esparso (so entra quem NAO e foto), ver
      // parseAnexoMeta. { [fileId]: { tipo: "video"|"doc", nome } }.
      fotosBeforeMeta: parseAnexoMeta(det["Foto Before Tipo"]),
      fotosImprovementMeta: parseAnexoMeta(det["Foto Improvement Tipo"]),
      isNew: isRecentlyCreated(det["Criado em"]),
    };
  });
}

// `filtro` (opcional): { status: "open"|"closed"|"", dept: "", investimento: "yes"|"no"|"" } -- filtra o
// conjunto de acoes ANTES de agregar, entao todo o resto da funcao (todas
// as abas do dashboard) automaticamente reflete o recorte, sem precisar de
// logica duplicada por aba.
export async function getDashboardStats(filtro = {}) {
  const investimentosPorNo = await readInvestimentosPorNo();
  let grupos = await readImprovementListRaw();
  if (filtro.status === "open") grupos = grupos.filter(({ acao }) => String(acao["Status"] || "").trim() !== "Closed");
  else if (filtro.status === "closed") grupos = grupos.filter(({ acao }) => String(acao["Status"] || "").trim() === "Closed");
  if (filtro.dept) {
    grupos = grupos.filter(
      ({ acao }) => String(acao["Dept. in charge"] || "Sem departamento").trim().replace(/\s+/g, " ") === filtro.dept
    );
  }
  if (filtro.investimento === "yes" || filtro.investimento === "no") {
    const querYes = filtro.investimento === "yes";
    grupos = grupos.filter(
      ({ acao }) => (String(acao["Investment"] || "").trim().toLowerCase() === "yes") === querYes
    );
  }
  ...
  const forecastPorSemanaDept = new Map(); // "YYYY-Www" -> Map(dept -> count)
  const forecastAcoesPorSemanaDept = new Map(); // "YYYY-Www" -> Map(dept -> [{no, item}]) -- pro tooltip da matriz
  ...
  grupos.forEach(({ acao }) => {
    const itens = investimentosPorNo.get(normNo(acao["No."])) || [];
    const status = String(acao["Status"] || "").trim();
    const statusRaw = status.toLowerCase();
    // "Atrasada" calculado aqui em JS, nao mais lendo a coluna "Time" da
    // planilha (formula viva baseada em HOJE() -- dependencia removida
    // junto com a migracao dos itens de investimento pra aba propria,
    // mesmo espirito de isoWeekKey() abaixo pro forecast).
    const deadlineParaAtraso = serialToDate(acao["New Deadline"]) || serialToDate(acao["Deadline"]);
    const isDelayed = statusRaw !== "closed" && Boolean(deadlineParaAtraso) && deadlineParaAtraso.getTime() < Date.now();
    ...
    if (statusRaw !== "closed") {
      ...
      if (d) {
        ...
        if (!forecastPorSemanaDept.has(semana)) forecastPorSemanaDept.set(semana, new Map());
        const porDeptoDaSemana = forecastPorSemanaDept.get(semana);
        porDeptoDaSemana.set(dept, (porDeptoDaSemana.get(dept) || 0) + 1);

        if (!forecastAcoesPorSemanaDept.has(semana)) forecastAcoesPorSemanaDept.set(semana, new Map());
        const acoesPorDeptoDaSemana = forecastAcoesPorSemanaDept.get(semana);
        if (!acoesPorDeptoDaSemana.has(dept)) acoesPorDeptoDaSemana.set(dept, []);
        acoesPorDeptoDaSemana.get(dept).push({ no: normNo(acao["No."]), item: acao["Item"] || "" });
      }
  ...
  const forecast = [...forecastPorSemana.entries()]
    ...
    .map(([semana, count]) => ({
      semana,
      count,
      // Map nao serializa no payload do Server Component -- vira objeto simples.
      porDepto: Object.fromEntries(forecastPorSemanaDept.get(semana) || []),
      porDeptoAcoes: Object.fromEntries(forecastAcoesPorSemanaDept.get(semana) || []),
    }));

  // departamentos que aparecem nessa janela de 12 semanas, do que tem mais
  // acoes previstas pro que tem menos -- define a ordem das linhas da
  // matriz semana x departamento no dashboard.
  const forecastDeptos = [...new Set(forecast.flatMap((f) => Object.keys(f.porDepto)))].sort((a, b) => {
    const totalA = forecast.reduce((acc, f) => acc + (f.porDepto[a] || 0), 0);
    const totalB = forecast.reduce((acc, f) => acc + (f.porDepto[b] || 0), 0);
...
// linhas pre-formatadas reservadas). O "No." e um placeholder "NEW-<base36
// do timestamp>" -- o numero de auditoria oficial da Hisense so e
// atribuido depois, por fora do site (mesmo comportamento documentado no
// fluxo antigo Excel/PPT). Base36 em vez do timestamp decimal cru: mesma
// unicidade (e so uma re-codificacao, sem perda), bem mais compacto pra
// caber no circulo do slide (ver .slide-badge).
// Investimento NAO e perguntado na criacao -- decisao explicita do usuario
// depois de um bug real (marcar "precisa" aqui nao criava nenhum item de
// investimento de verdade, so a flag, deixando a acao num estado
// inconsistente). Quem precisar de investimento adiciona o item depois,
// em Editar Ação -- isso ja atualiza a flag automaticamente.
export async function createAcao({ item, dept, person, occur, deadline, status }) {
  if (!item || !String(item).trim()) throw new Error("Item é obrigatório.");

  const grupos = await readImprovementListRaw();
  const lastRow = grupos.length ? Math.max(3, ...grupos.map((g) => g.row)) : 3; // linha 3 = cabecalho
  const novaLinha = lastRow + 1;
  const no = `NEW-${Date.now().toString(36).toUpperCase()}`;
...
// mesmo padrao de updatePassoField, escreve so a celula pedida na aba
// PPT_Investimentos.
export async function updateInvestmentItemField(row, field, value) {
  ...
  if (!rowNum || rowNum < 2) throw new Error("Linha de item de investimento inválida.");

  const sheets = getSheets();
  await sheets.spreadsheets.values.update({
    spreadsheetId: SHEET_ID,
    range: `${TAB_INVESTIMENTOS}!${col}${rowNum}`,
    valueInputOption: "USER_ENTERED",
    requestBody: { values: [[value]] },
  });
}

// Adiciona um item de investimento a uma acao -- mesmo padrao de addPasso():
// so um APPEND de linha nova em PPT_Investimentos, ligada pela coluna
// "No.". Funciona igual pro 1o, 2o ou 10o item da mesma acao, sem merge
// nem insercao de linha no meio de dado existente (ver o comentario no
// topo do arquivo sobre a migracao -- essa e exatamente a razao dela).
export async function addInvestimentoItem(no, { item, quantity, unitCost, supplier }) {
  if (!item || !String(item).trim()) throw new Error("Item é obrigatório.");

  const [grupos, { rows: itensExistentes }] = await Promise.all([
    readImprovementListRaw(),
    readSimpleTab(TAB_INVESTIMENTOS),
  ]);
  const alvo = grupos.find((g) => normNo(g.acao["No."]) === normNo(no));
  if (!alvo) throw new Error(`Ação ${no} não encontrada.`);

  // Coluna "#" e numerada corrida em TODA a planilha (historico
  // pre-migracao), nao reinicia por acao -- mantido pra continuar a
  // sequencia antiga.
  const numero = itensExistentes.reduce((max, r) => Math.max(max, Number(r["#"]) || 0), 0) + 1;
  const novaLinha = itensExistentes.length ? Math.max(...itensExistentes.map((r) => r._row)) + 1 : 2;

  const qtd = Number(quantity) || 1;
  const custoUn = Number(unitCost) || 0;
  const totalBRL = Math.round(qtd * custoUn * 100) / 100;
  const valores = {
    A: no,
    B: numero,
    C: item,
  ...
  const data = Object.entries(valores)
    ...
    .map(([col, v]) => ({ range: `${TAB_INVESTIMENTOS}!${col}${novaLinha}`, values: [[v]] }));
  ...
  if (String(alvo.acao["Investment"] || "").trim().toLowerCase() !== "yes") {
    await sheets.spreadsheets.values.update({
      spreadsheetId: SHEET_ID,
      range: `${TAB_IMPROVEMENT}!S${alvo.row}`,
      valueInputOption: "USER_ENTERED",
...
// "Remove" um item de investimento limpando a linha (mesmo padrao de
// deletePasso -- nao desloca as linhas de baixo).
export async function deleteInvestimentoItem(row) {
  const rowNum = Number(row);
  if (!rowNum || rowNum < 2) throw new Error("Linha de item de investimento inválida.");
  ...
  await sheets.spreadsheets.values.clear({
    spreadsheetId: SHEET_ID,
    range: `${TAB_INVESTIMENTOS}!A${rowNum}:R${rowNum}`,
    requestBody: {},
  });
}

// Troca o ID (coluna No.) de uma acao, propagando o vinculo pras abas que
// guardam "No." como chave propria (PPT_Detalhes = 1 linha, PPT_Passos e
// PPT_Investimentos = N linhas cada) -- sem isso a troca "descolaria"
// fotos/plano de acao/itens de investimento da acao.
export async function renameAcaoId(oldNo, newNo) {
  ...
  const [detalhes, passos, investimentos] = await Promise.all([
    readSimpleTab(TAB_DETALHES),
    readSimpleTab(TAB_PASSOS),
    readSimpleTab(TAB_INVESTIMENTOS),
  ]);
  ...
  const investLinhas = investimentos.rows.filter((r) => normNo(r["No."]) === normNo(oldNo));
  ...
  investLinhas.forEach((it) => data.push({ range: `${TAB_INVESTIMENTOS}!A${it._row}`, values: [[novo]] }));
...
// "soft delete" usado no resto do projeto) a linha dela na Improvement
// List, na PPT_Detalhes, todas as linhas dela na PPT_Passos, e todas as
// linhas dela na PPT_Investimentos.
export async function deleteAcao(no) {
  ...
  const investLinhas = investimentos.rows.filter((r) => normNo(r["No."]) === normNo(no));

  const sheets = getSheets();
  const data = [
    { range: `${TAB_IMPROVEMENT}!B${alvo.row}:U${alvo.row}` },
    ...(detLinha ? [{ range: `${TAB_DETALHES}!A${detLinha._row}:Z${detLinha._row}` }] : []),
    ...passosLinhas.map((p) => ({ range: `${TAB_PASSOS}!A${p._row}:F${p._row}` })),
    ...investLinhas.map((it) => ({ range: `${TAB_INVESTIMENTOS}!A${it._row}:R${it._row}` })),
  ];
