...
const _sheetIdCache = new Map(); // titulo da aba -> sheetId numerico (gid), pra chamadas de estrutura (inserir linha, mesclar)
async function getSheetIdByTitle(title) {
  if (_sheetIdCache.has(title)) return _sheetIdCache.get(title);
  const sheets = getSheets();
  const meta = await sheets.spreadsheets.get({
    spreadsheetId: SHEET_ID,
    fields: "sheets.properties(sheetId,title)",
  });
  (meta.data.sheets || []).forEach((s) => _sheetIdCache.set(s.properties.title, s.properties.sheetId));
  if (!_sheetIdCache.has(title)) throw new Error(`Aba "${title}" não encontrada na planilha.`);
  return _sheetIdCache.get(title);
}
...
// Coluna de "#" (V) e numerado corrido em TODA a planilha, nao reinicia
// por acao -- por isso o maximo e calculado sobre todos os grupos.
function proximoNumeroInvestimento(grupos) {
  let maxNum = 0;
  grupos.forEach((g) => g.itens.forEach((it) => { maxNum = Math.max(maxNum, Number(it["#"]) || 0); }));
  return maxNum + 1;
}

// Escreve um item de investimento (V:AL) numa linha ja existente e vazia
// (a linha-ancora da acao, quando ela ainda nao tem nenhum item).
async function escreverItemNaLinha(row, numero, { item, quantity, unitCost, supplier }) {
  const qtd = Number(quantity) || 1;
  const custoUn = Number(unitCost) || 0;
  const totalBRL = Math.round(qtd * custoUn * 100) / 100;
  const valores = { V: numero, W: item, Y: qtd, Z: custoUn, AA: totalBRL, AF: supplier || "" };
  const data = Object.entries(valores)
    .filter(([, v]) => v !== "" && v !== undefined)
    .map(([col, v]) => ({ range: `${TAB_IMPROVEMENT}!${col}${row}`, values: [[v]] }));
  const sheets = getSheets();
  await sheets.spreadsheets.values.batchUpdate({
    spreadsheetId: SHEET_ID,
    requestBody: { valueInputOption: "USER_ENTERED", data },
...
// Adiciona um item de investimento a uma acao. O PRIMEIRO item de cada acao
// escreve direto no bloco V:AL da propria linha-ancora (que fica vazio
// enquanto a acao nao tem nenhum item -- nao precisa inserir linha nem
// mexer em merge). Do SEGUNDO item em diante, nao ha linha "sobrando":
// insere uma linha fisica nova logo apos o ultimo item existente (empurra
// tudo que vem depois pra baixo, igual acontece hoje manualmente pra
// acoes com varios itens) e reaplica a mescla do bloco Acao (B:U) pra
// cobrir a linha nova, mantendo a leitura correta (readImprovementListRaw
// so olha a coluna "No." pra decidir onde comeca/continua um grupo -- a
// mescla e so visual, nao afeta a leitura, mas mante o aspecto da planilha
// consistente com o resto pra quem edita direto nela).
export async function addInvestimentoItem(no, { item, quantity, unitCost, supplier }) {
  ...
  const numero = proximoNumeroInvestimento(grupos);
  const sheets = getSheets();

  if (alvo.itens.length === 0) {
    await escreverItemNaLinha(alvo.row, numero, { item, quantity, unitCost, supplier });
  ...
  // Ja tem pelo menos 1 item -- insere uma linha logo apos o ultimo.
  const ultimaLinhaAtual = alvo.itens[alvo.itens.length - 1]._row;
  const novaLinha = ultimaLinhaAtual + 1;
  const sheetId = await getSheetIdByTitle(TAB_IMPROVEMENT);

  // A aba "Improvement List" tem 5 colunas congeladas (A:E) -- o Sheets
  // NAO deixa mesclar um intervalo que atravessa a fronteira congelado/nao
  // congelado numa unica chamada ("You can't merge frozen and non-frozen
  // columns"). O bloco Acao (B:U) cai dos dois lados dessa fronteira
  // (B:E congelado, F:U nao), entao a mescla precisa ser 2 pedidos
  // separados, um pra cada lado -- mesmo par (unmerge+merge) de antes, so
  // dividido no ponto exato onde o congelamento termina.
  const FROZEN_ATE_COL = 5; // colunas A..E (indices 0..4) congeladas
  const blocoAcaoRanges = [
    { startColumnIndex: 1, endColumnIndex: FROZEN_ATE_COL }, // B:E, dentro da area congelada
    { startColumnIndex: FROZEN_ATE_COL, endColumnIndex: 21 }, // F:U, fora da area congelada
  ];
  const mergeRequests = blocoAcaoRanges.flatMap(({ startColumnIndex, endColumnIndex }) => [
    {
      unmergeCells: {
        range: { sheetId, startRowIndex: alvo.row - 1, endRowIndex: ultimaLinhaAtual, startColumnIndex, endColumnIndex },
      },
    },
    {
      mergeCells: {
        range: { sheetId, startRowIndex: alvo.row - 1, endRowIndex: novaLinha, startColumnIndex, endColumnIndex },
        mergeType: "MERGE_ALL",
      },
    },
  ]);
  ...
  await sheets.spreadsheets.batchUpdate({
    ...
    requestBody: {
      requests: [
        {
          insertDimension: {
            range: { sheetId, dimension: "ROWS", startIndex: novaLinha - 1, endIndex: novaLinha },
  ...
  await escreverItemNaLinha(novaLinha, numero, { item, quantity, unitCost, supplier });
  if (String(alvo.acao["Investment"] || "").trim().toLowerCase() !== "yes") {
    await sheets.spreadsheets.values.update({
      spreadsheetId: SHEET_ID,
      range: `${TAB_IMPROVEMENT}!S${alvo.row}`,
