...
async function medirFoto(fileId, rotacoes, loadPhoto) {
  if (!fileId || !loadPhoto) return null;
  ...
  return { data: foto.data, rotate, ratio: larguraEfetiva / alturaEfetiva };
}
...
// os 2 primeiros IDs da lista sao as "principais" (fileira de cima, mais
// alta), o resto vira uma fileira de miniaturas embaixo. Mesma ordem que
// o site mostra. Cada fileira usa layout justificado (ver acima).
async function addPhotoGroup(slide, ids, rotacoes, x, y, w, h, loadPhoto) {
  const lista = (ids || []).filter(Boolean);
  if (!lista.length) return;

  const carregadas = await Promise.all(lista.map((id) => medirFoto(id, rotacoes, loadPhoto)));
