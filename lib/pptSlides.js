...
export async function addActionSlide(pptx, acao, loadPhoto) {
  ...
  const headerRow = ["#", "Action", "Owner", "Due", "Status"].map((text, i) => ({
    text,
    options: {
  ...
  // imprime so a foto principal de cada lado -- fotos extras (multiplas por
  // lado, ver Slide.js) aparecem no site mas nao entram no PPT exportado,
  // pra nao espremer o layout ainda mais.
  await addPhoto(slide, acao.fotoBeforeId, 0.35, photoY, 6.0, photoH, loadPhoto);
  await addPhoto(slide, acao.fotoImprovementId, 6.65, photoY, 6.0, photoH, loadPhoto);
}
