...
export function addOverviewSlide(pptx, stats, lang = "en") {
  ...
  // Donut de status (Closed vs Open) -- esquerda. O donut nativo nao
  // desenha texto no centro do anel nem uma legenda com contagem/%, entao
  // os dois sao desenhados por cima na mao (mesmo visual do SVG da tela:
  // total grande no meio do anel + legenda com "Status — N (X%)").
  const chartY = 2.3;
  const chartH = 3.55;
  const notaH = stats.potencialFechamento ? 0.55 : 0;
  const legendaH = 0.26;
  const donutH = chartH - 0.3 - legendaH - notaH;
  ...
  const donutCenterY = chartY + 0.3 + donutH / 2;
  slide.addText(
    [
      { text: String(stats.total) + "\n", options: { fontSize: 22, bold: true, color: TEXT_DARK } },
      { text: txt(s(lang, "pptDeck.actionsSuffix")), options: { fontSize: 9, color: "6B6878" } },
    ],
    { x: donutCenterX - 1, y: donutCenterY - 0.4, w: 2, h: 0.8, align: "center", valign: "middle", fontFace: "Arial" }
  );
  ...
  const legendaY = chartY + 0.3 + donutH + 0.04;
  const legendaItens = [
  ...
  legendaItens.forEach(([cor, texto]) => {
    slide.addShape("rect", { x: legendaX, y: legendaY + 0.05, w: 0.12, h: 0.12, fill: { color: cor }, line: { type: "none" } });
    slide.addText(txt(texto), { x: legendaX + 0.2, y: legendaY, w: 2.4, h: legendaH, fontSize: 8.5, color: TEXT_DARK, fontFace: "Arial", valign: "middle" });
    legendaX += 2.6;
  });
  if (stats.potencialFechamento) {
    ...
    slide.addText(
      ...
      {
        x: 0.35, y: legendaY + legendaH + 0.02, w: 5.8, h: notaH,
  ...
  // 10 departamentos pra caber num unico slide sem espremer demais. Sem
  // eixo de valor (a tela tambem nao mostra régua nenhuma, so a barra +
  // numero no fim dela) -- alem de bater com o visual da tela, evita o
  // formato "General" da barra de valor renderizar torto em alguns
  // visualizadores (aparecia como "1900r0oI", herdado do jeito que o
  // OOXML trata eixo numerico sem formato explicito).
