...
// Video entra pela CAPA (um quadro extraido dele no navegador na hora do
// upload e guardado como imagem no Drive -- ver extrairCapaDeVideo em
// components/PresentationClient.js): a capa e carregada e medida como
// qualquer foto, e ganha um selo de play por cima na hora de desenhar.
// Video sem capa (enviado antes desse recurso existir, ou cuja extracao
// falhou) cai no placeholder de antes: proporcao 16:9 e retangulo escuro.
async function medirFoto(fileId, tipo, rotacoes, loadPhoto, posterId) {
  if (!fileId) return null;
  if (tipo === "video") {
    if (posterId && loadPhoto) {
      const capa = await loadPhoto(posterId);
      if (capa && capa.data && capa.width && capa.height) {
        return { tipo: "video", data: capa.data, rotate: 0, ratio: capa.width / capa.height };
      }
    }
    return { tipo: "video", rotate: 0, ratio: 16 / 9 };
  }
...
function desenharFotoNaCaixa(slide, foto, boxX, boxY, boxW, boxH) {
  if (foto.tipo === "video") {
    ...
    // Multilaser pra video). Com capa, desenha o quadro extraido; sem
    // capa, o retangulo escuro de antes. Nos dois casos vai um selo de
    // play por cima, pra ficar claro que aquilo e um video e nao uma foto.
    if (foto.data) {
      slide.addImage({ data: foto.data, x: boxX, y: boxY, w: boxW, h: boxH });
    } else {
      slide.addShape("rect", { x: boxX, y: boxY, w: boxW, h: boxH, fill: { color: "17151F" }, line: { type: "none" } });
    }
    const triSize = Math.min(boxW, boxH) * 0.3;
    // Disco escuro atras do play: sobre uma capa clara o triangulo branco
    // sozinho sumiria (mesma solucao usada na tela, ver .tem-capa no CSS).
    if (foto.data) {
      const discoSize = triSize * 1.9;
      slide.addShape("ellipse", {
        x: boxX + boxW / 2 - discoSize / 2, y: boxY + boxH / 2 - discoSize / 2,
        w: discoSize, h: discoSize,
...
async function addPhotoGroup(slide, ids, rotacoes, meta, x, y, w, h, loadPhoto) {
  ...
  const carregadas = await Promise.all(
    lista.map((id) => medirFoto(id, meta?.[id]?.tipo || "foto", rotacoes, loadPhoto, meta?.[id]?.poster))
  );
