...
// O deck baixado e um documento oficial e fica sempre no formato original
// DD/MM/AAAA, independente do idioma escolhido na tela (mesma logica ja
// aplicada ao texto: rotulos ficam em ingles, dado real nao muda de cara
// conforme o viewer troca de idioma). `acao.occur`/`acao.deadline` chegam
// em ISO (YYYY-MM-DD) do servidor -- so reformata aqui, na hora de desenhar.
...
// esticar a imagem. `rotate` (0/90/180/270) reflete o giro aplicado no
// site (ver PresentationClient.js rotacionarFoto) -- gira em torno do
// centro da caixa, igual ao "transform: rotate()" da tela.
async function addPhoto(slide, fileId, x, y, w, h, loadPhoto, rotate) {
...
async function addPhotoGroup(slide, ids, rotacoes, x, y, w, h, loadPhoto) {
  ...
  for (let i = 0; i < nP; i++) {
    await addPhoto(slide, principais[i], x + i * (boxWP + gap), y, boxWP, alturaPrincipais, loadPhoto, rotacoes?.[principais[i]]);
  }
  ...
  for (let i = 0; i < nE; i++) {
    await addPhoto(slide, extras[i], x + i * (boxWE + gap), yExtras, boxWE, alturaExtras, loadPhoto, rotacoes?.[extras[i]]);
  }
}
...
export async function addActionSlide(pptx, acao, loadPhoto) {
  ...
  const metaY = 1.12;
  const metaCols = [
    ["ACTION LEADER", `${txt(acao.person) || "—"}${acao.dept ? ` (${txt(acao.dept)})` : ""}`],
    ["OCCUR. DATE", isoToBR(txt(acao.occur)) || "—"],
    ["DEADLINE", isoToBR(txt(acao.deadline)) || "—"],
    ["INVESTMENT", txt(acao.investment) || "—"],
  ];
  ...
  const descCols = [
    ["DESCRIPTION", acao.description, descW1],
    ["EXPECTATION", acao.expectation, descWResto],
    ["ABRANGENCY", acao.abrangency, descWResto],
  ];
  let cx = 0.35;
  ...
  const bodyRows = steps.map((st) =>
    st.map((v, i) => {
      ...
      return {
        text: isDate ? isoToBR(txt(v)) || txt(v) : txt(v),
        options: {
  ...
  await addPhotoGroup(slide, acao.fotosBefore, acao.fotosBeforeRotacao, 0.35, photoY, 6.0, photoH, loadPhoto);
  await addPhotoGroup(slide, acao.fotosImprovement, acao.fotosImprovementRotacao, 6.65, photoY, 6.0, photoH, loadPhoto);
}
