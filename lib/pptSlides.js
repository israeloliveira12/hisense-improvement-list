...
import { translate as s, translateValue as sv, formatDateISO } from "./i18n";
...
export async function addActionSlide(pptx, acao, loadPhoto, lang = "en") {
  ...
  // investimento: usa o campo ESTRUTURADO (total + situacao), nao a string
  // pronta em ingles -- a situacao (Approved/Declined/Pending) e traduzida
  // pro idioma do deck igual o resto.
  const investValor = acao.investmentInfo
    ? `R$ ${(acao.investmentInfo.total || 0).toLocaleString("pt-BR", { minimumFractionDigits: 2, maximumFractionDigits: 2 })} · ${sv(lang, acao.investmentInfo.situacao)}`
...
export function addOverviewSlide(pptx, stats, lang = "en") {
  ...
  // Donut de status (Closed vs Open) -- esquerda.
  const chartY = 2.3;
  const chartH = 3.55;
  ...
  slide.addChart(
    ...
    {
      x: 0.35, y: chartY + 0.3, w: 5.8, h: chartH - 0.3,
