...
export function addActionSlide(pptx, acao) {
  ...
  // v1 imprime so a foto principal de cada lado -- fotos extras (multiplas
  // por lado, ver Slide.js) aparecem no site mas ainda nao entram no PPT
  // exportado, pra nao espremer o layout ainda mais.
  return Promise.all([
...
function addCoverSlide(pptx, { geradoEm }) {
  const slide = pptx.addSlide();
